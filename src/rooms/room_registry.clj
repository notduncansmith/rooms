...
(defn create-room
  ([reg room-id] (if (rooms-above-max? reg) false (assoc-in reg [:rooms room-id] (room/empty-room))))
  ([reg] (create-room reg (ulid/ulid))))

(defn remove-room [reg room-id] (update reg :rooms dissoc room-id))

(defn add-lobby-user [reg user-id]
  (if (lobby-above-max? reg) false (update reg :lobby conj user-id)))

(defn remove-lobby-user [reg id] (update reg :lobby disj id))
